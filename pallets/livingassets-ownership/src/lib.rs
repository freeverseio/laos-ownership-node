...
///
/// ```
/// use your_pallet::LivingAssetsOwnership;
///
/// // assuming `Pallet` implements `LivingAssetsOwnership`
/// let collection_id = 1;
/// let account_id = ...; // assume we have a valid account_id
/// 
/// // create a new collection and assign ownership
/// Pallet::create_collection(collection_id, account_id.clone()).expect("Failed to create collection");
///
/// // retrieve the owner of the collection
/// assert_eq!(Pallet::owner_of_collection(collection_id), Some(account_id));
/// ```
///
/// # Errors
///

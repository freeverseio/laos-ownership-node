...
mod tests {
	...
	fn check_convert_asset_id_to_owner() {
		let value = AssetId(U256::from(5));
		let expected_address = H160::from_low_u64_be(5);
		assert_eq!(value.initial_owner(), expected_address);
	}
	...
	fn check_two_assets_same_owner() {
		...
		assert_eq!(AssetId(asset1).initial_owner(), expected_address);
		assert_eq!(AssetId(asset2).initial_owner(), expected_address);
	}
}

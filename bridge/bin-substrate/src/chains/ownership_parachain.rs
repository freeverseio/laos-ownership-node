...
use laos_runtime as ownership_parachain_runtime;
use relay_ownership_parachain_client::OwnershipParachain;
use relay_substrate_client::SimpleRuntimeVersion;
use xcm_executor::traits::ExportXcm;

impl CliEncodeMessage for OwnershipParachain {
	fn encode_wire_message(
		target: xcm::v3::NetworkId,
		at_target_xcm: xcm::v3::Xcm<()>,
	) -> anyhow::Result<Vec<u8>> {
		anyhow::ensure!(
			target == CustomNetworkId::Evochain.as_network_id(),
			anyhow::format_err!("Unsupported target chain: {:?}", target)
		);

		Ok(ownership_parachain_runtime::evochain_messages::ToEvochainBlobExporter::validate(
			target,
			0,
	...
	fn encode_execute_xcm(
		...
		type XcmCall = relay_ownership_parachain_client::runtime_types::pallet_xcm::pallet::Call;
